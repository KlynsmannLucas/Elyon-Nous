...
export const AuditorInputSchema = z.object({
  ...
  niche: z
    .string()
    .describe(
      "Nicho do cliente (ex: 'odontologia', 'e-commerce', 'móveis planejados')"
    ),
...
export interface AuditorOutput {
  agent: "auditor";
  score_conta: number; // 0-100
  grade: string;
  diagnostico: string[];
  erros_criticos: string[];
  gargalos: Array<{ rank: number; titulo: string; descricao: string; impacto: string }>;
  oportunidades: Array<{ titulo: string; descricao: string; potencial: string }>;
  resumo_executivo: string;
  plano_acao: {
    curto: Array<{ acao: string; como: string; impacto: string }>;
    medio: Array<{ acao: string; como: string; impacto: string }>;
    longo: Array<{ acao: string; como: string; impacto: string }>;
  };
  real_metrics: {
    totalSpend: number;
    totalLeads: number;
...
function buildAuditPrompt(input: AuditorInput): string {
  const {
    clientName,
    niche,
    budget = 0,
  ...
  if (highCTR.length > 0)
    anomalies.push(
      `⚠ CTR SUSPEITO (>10%): ${highCTR.map((c) => `${c.name} (${c.ctr}%)`).join(", ")} — possível bot traffic ou erro de evento`
    );

  const absurdROAS = allCampaigns.filter((c) => (c.roas || 0) > 40);
  if (absurdROAS.length > 0)
    anomalies.push(
      `⚠ ROAS IMPOSSÍVEL (>40×): ${absurdROAS.map((c) => `${c.name} (${c.roas}×)`).join(", ")} — provável erro no valor de conversão`
    );

  const wasteCamps = allCampaigns.filter(
    (c) => (c.spend || 0) > totalSpend * 0.1 && (c.leads || 0) === 0
  );
  if (wasteCamps.length > 0)
    anomalies.push(
      `🔴 DESPERDÍCIO CRÍTICO: ${wasteCamps.map((c) => `${c.name} (R$${c.spend?.toFixed(0)})`).join(", ")} — investimento alto SEM resultado`
    );

  const highFreq = allCampaigns.filter((c) => (c.frequency || 0) > 4);
  if (highFreq.length > 0)
    anomalies.push(
      `⚠ FADIGA CRIATIVA: ${highFreq.map((c) => `${c.name} (freq ${c.frequency?.toFixed(1)})`).join(", ")}`
    );
  ...
  const rankingText =
    ranked.length > 0
      ? `=== RANKING POR EFICIÊNCIA ===\n${ranked
          .map((c, i) => {
            const cpa =
              (c.leads || 0) > 0 ? Math.round((c.spend || 0) / (c.leads || 1)) : null;
            const eff =
              cpa === null
                ? "⛔ SEM CONVERSÃO"
  ...
  const metaSummary = hasMeta
    ? `=== META ADS ===\nGasto: R$${(metaTotals?.spend || 0).toFixed(2)} | CTR: ${metaTotals?.ctr || 0}% | CPL: R$${metaTotals?.cpl || 0} | ROAS: ${metaTotals?.roas || 0}× | Leads: ${metaTotals?.leads || 0}\n${metaCampaigns
        ...
        .map(
          (c) =>
            `  [${c.status || "ACTIVE"}] ${c.name} — Gasto R$${(c.spend || 0).toFixed(2)} | Leads ${c.leads || 0} | CPL R$${(c.cpl || 0).toFixed(2)} | CTR ${(c.ctr || 0).toFixed(2)}% | Freq ${(c.frequency || 0).toFixed(1)}`
        )
        .join("\n")}\n`
    : "";
  ...
  const googleSummary = hasGoogle
    ? `=== GOOGLE ADS ===\nGasto: R$${(googleTotals?.spend || 0).toFixed(2)} | CTR: ${googleTotals?.ctr || 0}% | CPL: R$${googleTotals?.cpl || 0} | Conversões: ${googleTotals?.leads || 0}\n${googleCampaigns
        ...
        .map(
          (c) =>
            `  [${c.status || "ACTIVE"}] ${c.name} — Gasto R$${(c.spend || 0).toFixed(2)} | Leads ${c.leads || 0} | CPL R$${(c.cpl || 0).toFixed(2)}`
        )
        .join("\n")}\n`
    : "";
...
export async function runAuditor(
  ...
  const result = await callLLMJson<Omit<AuditorOutput, "agent" | "generated_at" | "source" | "real_metrics">>(
    server,
    {
      user: prompt,
      maxTokens: 6000,
