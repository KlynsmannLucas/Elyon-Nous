...
export const ReportInputSchema = z.object({
  ...
  format: z
    ...
    .default("executive")
    .describe(
      "Formato do relatório: executive (C-level), tactical (operacional), investor (pitch)"
    ),
});
...
export interface ReportOutput {
  ...
  cliente: string;
  nicho: string;
  score_geral: number; // 0-100 ponderado
  grade: string;
  sumario_executivo: string;
  kpis_chave: Array<{ nome: string; valor: string; status: string; benchmark: string }>;
  principais_descobertas: string[];
  riscos_criticos: string[];
  oportunidades_principais: string[];
  plano_acao: ReportAction[];
  projecao_90_dias: {
    cenario_base: string;
    cenario_otimizado: string;
    premissas: string[];
  };
  proximos_passos_imediatos: string[]; // próximos 7 dias
  slide_pitch: string; // 3-5 bullets para pitch rápido
  generated_at: string;
  source: "ai";
}

// ── Helpers de contexto ─────────────────────────────────────────────────────
function auditorCtx(a: any): string {
  if (!a) return "";
...
${(a.gargalos || [])
  .slice(0, 3)
  .map((g: any) => `  [${g.rank}] ${g.titulo}: ${g.descricao} → ${g.impacto}`)
  .join("\n")}

Oportunidades (top 3):
${(a.oportunidades || [])
  .slice(0, 3)
  .map((o: any) => `  • ${o.titulo} — ${o.potencial}`)
  .join("\n")}

Plano (auditor):
- Curto: ${(a.plano_acao?.curto || []).map((x: any) => x.acao).join("; ")}
- Médio: ${(a.plano_acao?.medio || []).map((x: any) => x.acao).join("; ")}
- Longo: ${(a.plano_acao?.longo || []).map((x: any) => x.acao).join("; ")}
...
function dataAnalystCtx(d: any): string {
  if (!d) return "";
...
Anomalias: ${(d.anomalias || []).map((a: any) => `${a.titulo} (${a.impacto})`).join(" | ")}

Segmentos vencedores: ${(d.segmentos_vencedores || []).map((s: any) => `${s.segmento} (${s.metrica})`).join(" | ")}
Segmentos perdedores: ${(d.segmentos_perdedores || []).map((s: any) => `${s.segmento} (${s.metrica})`).join(" | ")}
...
Inteligência de mercado: ${(d.inteligencia || []).slice(0, 3).map((i: any) => i.titulo).join(" | ")}`;
}

function estrategistaCtx(s: any): string {
  if (!s) return "";
...
${(s.priority_ranking || [])
  ...
  .map(
    (r: any) =>
      `  ${r.priority}. ${r.channel} — ${r.budget_pct}% (R$${r.budget_brl}) | CPL R$${r.cpl_min}-${r.cpl_max} | Leads ${r.leads_min}-${r.leads_max}`
  )
  .join("\n")}
...
function copywriterCtx(c: any): string {
  if (!c) return "";
  return `=== COPYWRITER ===
Big idea: ${c.big_idea || "—"}
Tom: ${c.tom_de_voz || "—"}
Variações geradas: ${(c.variacoes || []).length}
Frameworks explorados: ${[...new Set((c.variacoes || []).map((v: any) => v.framework))].join(", ")}

Headlines exemplo:
${(c.variacoes || [])
  .slice(0, 5)
  .map((v: any) => `  [${v.framework}] ${v.titulo} → ${v.cta}`)
  .join("\n")}

Plano de teste criativo: ${(c.plano_de_teste || []).map((p: any) => `${p.fase}: ${p.teste}`).join(" | ")}`;
}

// ── Prompt builder ──────────────────────────────────────────────────────────
function buildReportPrompt(input: ReportInput): string {
  const { client, auditor, data_analyst, estrategista, copywriter, format } =
    input;

  const formatInstructions = {
    executive:
      "Tom: C-level. Linguagem de sala de reunião. Números antes de opiniões. Foco em impacto financeiro e decisões de budget.",
    tactical:
      "Tom: operacional. Foco em EXECUÇÃO — quem faz o quê, em qual ordem, com qual métrica. Menos narrativa, mais checklist.",
    investor:
      "Tom: pitch para comitê. Demonstrar oportunidade, tração e plano de capital. Números de mercado, unit economics e projeção.",
  };
...
export async function runReport(
  ...
  const result = await callLLMJson<
    Omit<ReportOutput, "agent" | "generated_at" | "source">
  >(server, {
    user: prompt,
    maxTokens: 8000,
  });
