...
export interface DataAnalystOutput {
  ...
  // Núcleo do PDF
  insights: string[];
  anomalias: Array<{ titulo: string; descricao: string; impacto: string }>;
  segmentos_vencedores: Array<{
    segmento: string;
    motivo: string;
    metrica: string;
  }>;
  segmentos_perdedores: Array<{
    segmento: string;
    motivo: string;
    metrica: string;
  }>;
  ...
  // Diagnóstico estratégico (portado de /api/diagnostic)
  health_score: number;
  grade: string;
  executive_summary: string;
  saude_financeira: {
    break_even_roas: number;
    cpl_maximo_lucrativo: number;
    ltv_estimado: number;
    cac_payback_meses: number;
  ...
  prontidao_para_escalar: {
    score: number;
    pode_escalar_agora: boolean;
    prerequisitos_faltando: string[];
  ...
  diagnostico_funil: {
    etapas: Array<{
      etapa: string;
      status: "saudavel" | "problema" | "nao_auditado";
      observacao: string;
    }>;
  ...
  recomendacao_principal: {
    titulo: string;
    descricao: string;
    acao_semana_1: string;
    acao_mes_1: string;
    acao_trimestre: string;
  };

  // Inteligência de mercado (portado de /api/intelligence)
  inteligencia: Array<{
    tipo: string;
    icone: string;
    titulo: string;
  ...
  benchmark_comparativo: {
    cpl_atual: number;
    cpl_benchmark: number;
    cpl_status: "excelente" | "bom" | "atencao" | "critico";
    roas_break_even: number;
    roas_bom_nicho: number;
    melhores_canais: string[];
    insights_nicho: string[];
  };
...
function computeUnitEconomics(input: DataAnalystInput) {
  const bench = getBenchmark(input.niche);
  const margin = (input.grossMargin ?? 40) / 100;
  const cvr =
    (input.conversionRate ?? (bench?.cvr_lead_to_sale ?? 0.1) * 100) / 100;
  ...
  const cacPayback =
    maxCPL > 0 ? +(maxCPL / (ticket * margin)).toFixed(1) : 1;
  return {
    bench,
    margin,
    cvr,
...
// ── Prompt builder ──────────────────────────────────────────────────────────
function buildDataAnalystPrompt(input: DataAnalystInput): string {
  const {
    clientName,
    niche,
    budget = 0,
  ...
  const historyCtx =
    campaignHistory.length > 0
      ? campaignHistory
          ...
          .map(
            (c) =>
              `• ${c.period} | ${c.channel} | Gasto: R$${c.budgetSpent ?? "?"} | Leads: ${c.leads ?? "?"} | CPL: R$${c.cplReal ?? "?"} | Vendas: ${c.conversions ?? "?"} | ${c.outcome?.toUpperCase() ?? "?"} | Funcionou: ${c.whatWorked || "—"} | Falhou: ${c.whatFailed || "—"}`
          )
          .join("\n")
  ...
  const channelsText =
    strategy?.recommended_channels_names?.join(", ") ||
...
export async function runDataAnalyst(
  ...
  const result = await callLLMJson<
    Omit<DataAnalystOutput, "agent" | "generated_at" | "source">
  >(server, {
    user: prompt,
    maxTokens: 8000,
  });
