...
export default function PerfilPageInner() {
  ...
  useEffect(() => {
    ...
    fetch('/api/stripe/invoices')
      .then(r => r.json())
      .then(d => { if (d.error) setInvoicesError(d.error); else setInvoices(d.invoices || []) })
  ...
  const handleAddPhone = async () => {
    ...
    try {
      ...
      setPhoneObj(ph); setOtpStep(true)
      setPhoneMsg('Código enviado via SMS. Digite abaixo para confirmar.')
    } catch (e: any) {
      setPhoneMsg(e.errors?.[0]?.longMessage || 'Erro ao adicionar telefone.')
  ...
  const handleVerifyPhone = async () => {
    ...
    try {
      ...
      await user.reload()
      setShowPhoneForm(false); setOtpStep(false); setPhoneInput(''); setOtpCode('')
  ...
  const handleChangePassword = async () => {
    ...
    setPwLoading(true)
    try {
      await (user as any).updatePassword({ currentPassword: pwForm.current, newPassword: pwForm.next, signOutOfOtherSessions: false })
      setPwSuccess(true); setPwMsg('Senha alterada com sucesso!')
      setPwForm({ current: '', next: '', confirm: '' })
    } catch (e: any) {
      setPwMsg(e.errors?.[0]?.longMessage || 'Erro ao alterar senha. Verifique a senha atual.')
  ...
  const handleSync = async () => {
    ...
    const data = await res.json()
    if (data.success) { setSyncMsg(`Plano "${data.plan}" sincronizado! Atualizando...`); setTimeout(() => window.location.reload(), 1500) }
    else setSyncMsg(data.message || 'Nenhuma assinatura encontrada.')
    setSyncLoading(false)
  }

  const planInfo    = effectivePlan ? PLAN_LABELS[effectivePlan] ?? null : null
  ...
  const formatCurrency = (amount: number, currency: string) => {
    ...
    return currency === 'brl' ? `R$${val.toFixed(2).replace('.', ',')}` : `$${val.toFixed(2)}`
  }
          ...
          <div className="bg-[#111114] border border-[#2A2A30] rounded-2xl p-6">
            ...
            {activeSection === 'dados' && (
              <div>
                ...
                <Field label="Membro desde"    value={effectiveCreatedAt} />
              ...
              <div>
                ...
                <div className="mb-6 p-4 bg-[#16161A] rounded-xl border border-[#2A2A30] space-y-3">
                  ...
                  {[
                    ...
                    { icon: '📊', label: 'Clientes',      desc: 'Dados dos clientes: nome, nicho, orçamento, métricas e estratégias' },
