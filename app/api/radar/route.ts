// app/api/radar/route.ts — RADAR DIÁRIO: vigia as contas (Meta + Google) dos
// ÚLTIMOS 7 DIAS e devolve "as coisas que precisam de você hoje", já ranqueadas
// por dinheiro: 🔴 vazamento, 🟡 risco, 🟢 oportunidade de escala. Cada alerta
// traz o valor em R$ e, quando aplicável, a ação executável (com aprovação).
//
// Baseline-aware: usa daily_metrics (a própria média da conta) para flagrar
// "CPL disparou vs sua média" — o sinal mais relevante do dia.
...
type Severity = 'leak' | 'risk' | 'opportunity'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const convRate = Number(body.convRate) || 0
    const breakeven = ticket > 0 && margin > 0 && convRate > 0
      ? Math.round(ticket * (margin / 100) * (convRate / 100)) : null
    ...
    try {
      ...
      metaAccountId = bodyAccountId || t.accountId
      if (metaAccountId) {
    ...
    // 🔴 Vazamento: gasto sem conversão (7d)
    ...
    if (waste.length) {
      ...
      alerts.push({
        severity: 'leak',
        title: `${brl(c.spend)} queimados sem conversão`,
        detail: waste.length > 1 ? `"${c.name}" e mais ${waste.length - 1} campanha(s) gastaram ${brl(wasteTotal)} em 7 dias sem 1 conversão.` : `"${c.name}" gastou sem gerar nenhum resultado em 7 dias.`,
        money: Math.round(wasteTotal), campaignId: c.id || undefined, campaignName: c.name, platform: 'meta', action: c.id ? 'pause' : undefined,
      })
    }

    // 🔴 Prejuízo: CPL acima do equilíbrio (cada lead custa mais do que o lucro)
    if (breakeven) {
      ...
      if (loss.length) {
        ...
        alerts.push({
          severity: 'leak',
          title: `"${c.name}" dá prejuízo (CPL ${brl(c.cpl)})`,
          detail: `Acima do seu equilíbrio (${brl(breakeven)}). Cada lead custa mais do que gera de lucro — ~${brl(extra)} a mais em 7 dias.`,
          money: extra, campaignId: c.id || undefined, campaignName: c.name, platform: 'meta', action: c.id ? 'pause' : undefined,
        })
      }
    }

    // 🟡 Risco: fadiga de criativo (frequência alta)
    const sat = camps.filter(c => c.freq >= 3.5).sort((a, b) => b.freq - a.freq)
    if (sat.length) {
      const c = sat[0]
      alerts.push({
        severity: 'risk',
        title: `"${c.name}" com fadiga (frequência ${c.freq.toFixed(1)}×)`,
        detail: `O público está vendo o mesmo anúncio demais — o CTR cai e o CPL sobe. Renove o criativo.`,
        money: 0, campaignName: c.name, platform: 'meta',
      })
    }

    // 🟢 Oportunidade: vencedora barata → escalar
    ...
    if (winThreshold) {
      ...
      if (winners.length) {
        ...
        alerts.push({
          ...
          title: `"${c.name}" rende barato (CPL ${brl(c.cpl)}) — escale`,
          detail: `Bem abaixo do seu alvo. Suba o orçamento ~20% enquanto o CPL segurar — dá pra colocar ~${brl(c.spend * 0.2)}/sem a mais com lucro.`,
          money: Math.round(c.spend * 0.2), campaignId: c.id || undefined, campaignName: c.name, platform: 'meta', action: c.id ? 'scale' : undefined,
        })
      }
    }

    // 🔴/🟡 Baseline da conta: CPL de ontem vs a própria média (daily_metrics)
      ...
      try {
        ...
        if (withCpl.length >= 4) {
          ...
          if (avg > 0 && yest.cpl! > avg * 1.25) {
            ...
            alerts.push({
              severity: deltaPct >= 50 ? 'leak' : 'risk',
              title: `CPL da conta disparou ${deltaPct}% vs sua média`,
              detail: `Ontem o CPL foi ${brl(yest.cpl!)} contra a sua média de ${brl(avg)}. Algo mudou — investigue antes que escale.`,
              money: extra > 0 ? extra : 0, deltaPct,
            })
          }
        }
    ...
    // Ranking: vazamento → risco → oportunidade; dentro do grupo, mais dinheiro primeiro.
    ...
    return NextResponse.json({ success: true, alerts: alerts.slice(0, 5), moneyAtRisk, analyzed: camps.length })
  } catch (e: any) {
