...
export default async function PortalPage({ params }: { params: { slug: string } }) {
  const data = await getPortal(params.slug)
  if (!data) notFound()

  const r = data.report_data
  const clientName  = data.client_name
  const agencyName  = r?.agencyName  || 'Agência'
  const showMetrics = r?.showMetrics  ?? true
  const showStrategy= r?.showStrategy ?? true
  const showActions = r?.showActions  ?? false
  const niche       = r?.niche  || ''
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080D1A', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      {/* Top bar */}
      <div style={{
        background: '#0F1629', borderBottom: '1px solid rgba(255,255,255,0.06)',
        padding: '14px 24px',
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          ...
          <p style={{ fontSize: '13px', color: '#64748B', margin: 0 }}>
            Relatório de performance · Atualizado em {new Date(createdAt).toLocaleDateString('pt-BR', { day: 'numeric', month: 'long', year: 'numeric' })}
          </p>
        </div>
        ...
        {showMetrics && (
          <section style={{ marginBottom: '28px' }}>
            <div style={{ fontSize: '11px', fontWeight: 700, color: '#64748B', letterSpacing: '0.1em', marginBottom: '12px', textTransform: 'uppercase' }}>
              Resumo da conta
            </div>
          ...
          <section style={{ marginBottom: '28px' }}>
            ...
            <div style={{ padding: '18px 20px', borderRadius: '12px', background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.2)' }}>
              ...
              <p style={{ fontSize: '13px', color: '#94A3B8', lineHeight: 1.6, margin: '0 0 12px' }}>
                ...
                {niche ? <> para o nicho <strong style={{ color: '#F1F5F9' }}>{niche}</strong></> : ''}. A estratégia
                prioriza CPL abaixo do benchmark de mercado com escalonamento progressivo de verba.
              </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {['Renovar criativos para a próxima semana', 'Expandir audiências lookalike para escalar volume', 'Otimizar landing page com base nos dados de CTR'].map((action, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', borderRadius: '10px', background: '#0F1629', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <div style={{ width: '22px', height: '22px', borderRadius: '6px', flexShrink: 0, background: 'rgba(124,58,237,0.10)', border: '1px solid rgba(124,58,237,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: 700, color: '#A78BFA' }}>
                    {i + 1}
                  </div>
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  return {
    title: `${data.client_name} — ${data.report_data?.agencyName || 'Agência'}`,
    description: `Relatório de performance para ${data.client_name}`,
  }
}
